...
solo si cumplen con la condición de turno nocturno adyacente.
"""
...
    asignar un turno válido. Retorna df con la columna 'Es_Nocturno'.
    """
    
...
        es_nocturno_flag = False # NUEVA BANDERA para el filtro solicitado
        
...
                # Establecer la bandera de turno nocturno
...
            'Es_Nocturno': es_nocturno_flag # NUEVO: Bandera para el filtro de extremos
        })

    return pd.DataFrame(resultados)

# --- 5. Nueva Función de Filtrado Post-Cálculo (Lógica Solicitada) ---

def aplicar_filtro_primer_ultimo_dia(df_resultado):
    """
    Aplica el filtro para conservar el primer y último día solo si cumplen
    con la condición de turno nocturno adyacente (dentro de los días con registro).
...
        # Ordenar por fecha para identificar correctamente los extremos
        df_worker = df_worker_group.sort_values(by='FECHA').copy()
        
        # Obtener la lista única y ordenada de fechas con marcaciones para este trabajador
...
        # Mapeo de fechas a la propiedad 'Es_Nocturno' (tomamos True si existe al menos una vez en ese día)
        # Agrupamos por la fecha de reporte (FECHA_DATE)
        date_to_nocturno = df_worker.groupby('FECHA_DATE')['Es_Nocturno'].any().to_dict()

        for current_date in unique_dates:
            
            # --- Regla A: Días Intermedios (No son ni el primero ni el último) ---
            if current_date > first_day and current_date < last_day:
                # Si es un día intermedio, se conserva TODA la información de ese día.
                rows_to_keep_indices.extend(df_worker[df_worker['FECHA_DATE'] == current_date].index.tolist())
                continue
                
            # --- Regla B & D: Primer Día (Incluye caso de día único) ---
            elif current_date == first_day:
                # El día adyacente para el primer día es el día siguiente
                day_after_first = current_date + timedelta(days=1)
                
                # Buscamos si el día siguiente es nocturno (dentro de los días que tienen registro)
                is_next_day_nocturno = date_to_nocturno.get(day_after_first, False)

                if current_date == last_day:
                    # Caso: Único Día (Primer y Último son iguales) - No se puede verificar D-1
                    # Si solo hay un día (len(unique_dates)==1), no tenemos registro del día anterior.
                    # Siguiendo la regla estricta (DEBEN cumplirse AMBAS condiciones), se excluye.
                    pass # Se excluye.
                else:
                    # Caso: Es el Primer día de muchos
                    if is_next_day_nocturno:
                        rows_to_keep_indices.extend(df_worker[df_worker['FECHA_DATE'] == current_date].index.tolist())
                
            # --- Regla C: Último Día (No es el primero) ---
            elif current_date == last_day:
                # El día adyacente para el último día es el día anterior
                day_before_last = current_date - timedelta(days=1)
                
                # Buscamos si el día anterior es nocturno
                is_prev_day_nocturno = date_to_nocturno.get(day_before_last, False)
                
                if is_prev_day_nocturno:
                    rows_to_keep_indices.extend(df_worker[df_worker['FECHA_DATE'] == current_date].index.tolist())
...
        # Cambiada 'data' a la hoja original de la que solía ser: 'BaseDatos Modificada'
...
        # --- Ejecutar el Cálculo (Genera resultados con columna 'Es_Nocturno') ---
