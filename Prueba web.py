...
        # Se omite para limpiar el reporte si no se pudo asignar una entrada (entrada_real es NaT).
...
def aplicar_filtro_primer_ultimo_dia(df_resultado):
    """
    APLICA FILTRO REQUERIDO:
    1. Días intermedios SIEMPRE se conservan.
    2. Primer Día (D_inicio): Se conserva si el registro en D_inicio corresponde a un Turno Nocturno.
    3. Último Día (D_fin): Se conserva SOLO si el registro en D_fin corresponde a un Turno Nocturno.
    """
    ...
    # La columna FECHA ya es datetime.date, la usamos directamente
    ...
    for id_trabajador, df_worker_group in df_filtrado.groupby('ID_TRABAJADOR'):
        ...
        # Obtener la lista única y ordenada de fechas de reporte con marcaciones para este trabajador
        ...
        # Mapeo de fechas a la propiedad 'Hay_Turno_Nocturno' (True si existe al menos un registro nocturno en ese día de reporte)
        # Esto nos indica si una jornada en ese día es nocturna.
        ...
        for current_date in unique_dates:
            ...
            is_nocturno_in_day = date_to_nocturno.get(current_date, False)
                
            # --- Regla B & C: Primer Día (Incluye caso de día único) ---
            if current_date == first_day:
                # Se conserva si el registro en D_inicio corresponde a un Turno Nocturno.
                if is_nocturno_in_day:
                    rows_to_keep_indices.extend(df_worker[df_worker['FECHA_DATE'] == current_date].index.tolist())
                continue
                
            # --- Regla D: Último Día (No es el primero) ---
            elif current_date == last_day:
                # Se conserva SOLO si el registro en D_fin corresponde a un Turno Nocturno.
                if is_nocturno_in_day:
                    rows_to_keep_indices.extend(df_worker[df_worker['FECHA_DATE'] == current_date].index.tolist())
                continue
    ...
    # Limpieza: Asegurarse de que la columna de trabajo se elimine del resultado final
    if 'Es_Nocturno' in df_final.columns:
        df_final.drop(columns=['Es_Nocturno'], inplace=True) 
...
st.write("Sube tu archivo de Excel para calcular las horas extra del personal. **Nota Importante:** El primer y último día del reporte solo se incluyen si alguno de sus turnos asignados es de tipo nocturno.")
...
            
            # --- APLICAR EL NUEVO FILTRO DE PRIMER Y ÚLTIMO DÍA (MODIFICADO) ---
