...
def aplicar_filtro_primer_ultimo_dia(df_resultado):
    ...
    Lógica Solicitada:
    - Primer Día: Mantener si es una entrada viable para turno nocturno.
    - Último Día: Mantener si es una salida viable para turno nocturno (Excluye el inicio de turno nocturno).
    """
        ...
        for current_date in unique_dates:
            ...
            # --- Primer Día ---
                ...
                if current_date == last_day:
                    ...
                    # Primer día de muchos: Mantenemos todos los turnos. La lógica de asignación de FECHA_CLAVE_TURNO
                    # ya agrupó las entradas de madrugada al día anterior, así que este día solo tendrá inicios de turno diurno
                    # o el inicio del nocturno (21:40). Mantenemos todos los resultados de este día.
                    rows_to_keep_indices.extend(df_worker[df_worker['FECHA_DATE'] == current_date].index.tolist())


            # --- Último Día ---
            elif current_date == last_day:
                # El último día solo debe contener las salidas (turnos diurnos o final de turno nocturno).
                # Excluimos cualquier registro que esté marcado como inicio de turno nocturno (Es_Nocturno=True),
                # ya que su salida estaría fuera del rango del reporte.
