...
# Se garantiza que no hay caracteres invisibles U+00A0
TURNOS = {
    "LV": { # Lunes a Viernes
...
# **Ajuste:** Tolerancia aumentada a 120 minutos (2 horas) para mayor flexibilidad en la asignación de turnos.
...
def calcular_turnos(df: pd.DataFrame, lugares_normalizados: list, tolerancia_minutos: int, tolerancia_llegada_tarde: int):
    ...
    for (id_trabajador, fecha_clave_turno), grupo in df.groupby(['ID_TRABAJADOR', 'FECHA_CLAVE_TURNO']):
        ...
        # **CORRECCIÓN CLAVE para mostrar todos los días:**
        # Regla 2: Solo verifica que la salida sea posterior a la entrada. SE ELIMINA LA RESTRICCIÓN DE DURACIÓN MÍNIMA (4 HORAS).
        if salida_real <= entrada_real:
            continue

        # Regla 3: Intenta asignar un turno programado (CON RESCATE)
        
        # 3.1: Intento con la tolerancia configurada (120 minutos)
        ...
        if turno_nombre is None:
            # 3.2: Intento de rescate con una tolerancia mayor (180 minutos = 3 horas)
            # Esto ayuda a capturar entradas muy tempranas o salidas muy tardías que están fuera del rango normal.
        ...
        # Añade el resultado (incluye días con Horas_Extra = 0)
        resultados.append({
            'NOMBRE': nombre,
...
st.title("📊 Calculadora de Horas Trabajadas y Extra")
st.write("Sube tu archivo de Excel para calcular las horas, mostrando todas las jornadas con turno asignado.")
...
if archivo_excel is not None:
    try:
        ...
        if not all(col in df_raw.columns for col in columnas):
            ...
            # Llama a la función principal de cálculo
            ...
            if not df_resultado.empty:
                # Lógica para preparar el DataFrame de salida
                ...
                st.subheader("Resultados de Horas Trabajadas y Horas Extra (Incluye días con 0 Horas Extra)")
                ...
                st.warning("No se pudieron asignar turnos o hubo inconsistencias en los registros que cumplieran los criterios de cálculo.")
